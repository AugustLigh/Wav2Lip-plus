...
def load_model(path):
    ...
    checkpoint = _load(path) # Loads the file

    # Check if the loaded checkpoint is a TorchScript module or a dictionary
    if isinstance(checkpoint, torch.jit.ScriptModule):
        print("Loaded checkpoint is a TorchScript module.")
        model = checkpoint
        # Assuming the ScriptModule is the model itself.
        # No need to manually load state_dict in this case.
    elif isinstance(checkpoint, dict) and "state_dict" in checkpoint:
        print("Loaded checkpoint is a dictionary.")
        model = Wav2Lip() # Create a new instance
        s = checkpoint["state_dict"]
        new_s = {}
        # Remove 'module.' prefix if present (often added by DataParallel)
        for k, v in s.items():
            new_s[k.replace('module.', '', 1)] = v
        model.load_state_dict(new_s)
    else:
        # Handle unexpected checkpoint format
        raise TypeError(f"Loaded checkpoint is of unexpected type: {type(checkpoint)}. Expected a dict or TorchScript module.")
